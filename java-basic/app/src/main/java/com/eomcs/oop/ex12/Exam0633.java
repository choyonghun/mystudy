...
public class Exam0633 {
  ...
  public static void main(String[] args) {
    ...
    // 이유 :
    // 1) isEmpty() 메서드는 스테틱 메서드가 아니기때문에 스태틱 메서드처럼 호출하면 오류가 발생해야 한다.
    // 2) isEmpty() 는 파라미터가 없기 때문에 String 값을 받을수 없다.
    // 그러나, 컴파일 오류가 발생하지 않는다.
    //
    // 인터페이스의 타입 파라미터에 전달하는 클래스와 메서드 레퍼런스의 클래스가 같을 때
    // 위 코드는 다음과 같이 변경된다.
    // => Predicate<String> p4 = (String value) -> { return value.isEmpty(value); };
    // => Predicate<String> p4 = value -> value.isEmpty(value);
