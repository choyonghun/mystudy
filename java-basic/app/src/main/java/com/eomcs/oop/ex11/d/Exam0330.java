...
class D3 {
  ...
  void m1() {
    ...
    // 그런데 X.f()를 호출하기 전에는 v3변수의 값을 바꿨다.
    // 물론 X 인스턴스 obm에는 이전의 v3값인 3이 저장되어 있겠지만,
    // 개발자 입장에서는 X.f()를 호출 할대 30으로 변경된 값을 사용할 것이라고 판단할수 있다.
    // 이런 혼란스러운 상황을 피하기 위해
    // 로컬 틀래스에서 enclosing 메서드의 변수를 사용할 때는
